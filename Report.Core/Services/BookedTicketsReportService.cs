...
{
    public class BookedTicketsReportService : Service<CustomerBookings>, IBookedTicketsReportService
    {

        public BookedTicketsReportService(IUnitOfWork unitOfWork) : base(unitOfWork)
        {

        }

        public async Task<ApiResponse<PaginatedList<BookedTicketReportViewModel>>> GetBookedTickets(GetBookedTicketReportViewModel model)
        {
            var response = new ApiResponse<PaginatedList<BookedTicketReportViewModel>> { Code = ApiResponseCodes.OK };
            ...
            var validationResult = new GetBookedTicketReportValidator().Validate(model);
            ...
            {
                var dataQuery = UnitOfWork.Repository<CustomerBookings>().GetAll()
                                .Where(d => d.BookingDate >= startDate && d.BookingDate <= endDate);

                if (!string.IsNullOrEmpty(model.ReferenceCode))
                {
                    //This code block will only filter records by ref code if it was specified
                    dataQuery = dataQuery.Where(d => d.RefCode.Equals(model.ReferenceCode));
                }

                return dataQuery.OrderBy(d => (DateTime)d.BookingDate)
                                .Select(d => (BookedTicketReportViewModel)d)
                               ;
            });
